...
//    }
   ...
   $sql = "INSERT INTO theory_test( name , address , post_code , phone , loc_test_center , near_center , licence_front_pic , licence_back_pic , cost_price)
   VALUES('{$name}','{$address}','{$postcode}','{$phone}','{$location_test_center}','{$near_test_center}','{$file_name}','{$file_name2}','{$cost_price}')";
   $result = mysqli_query($con,$sql) or die();
   
   header("Location: http://localhost/pass-2/theory_test.php");
}
