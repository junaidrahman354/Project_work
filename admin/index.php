...
<?php 
    $select = "select COUNT(*) as total from instructor_registration ";
    $res = mysqli_query($con, $select);
    $data = mysqli_fetch_array($res);
    if($data['total'] == ''){
        $data['total'] = 0;
...
<?php 
    $select = "select COUNT(*) as total from instructor_registration where user_status = 'Active'";
    $res = mysqli_query($con, $select);
    $data = mysqli_fetch_array($res);
    if($data['total'] == ''){
        $data['total'] = 0;
...
?>
                    <div class="col-sm-6 col-xl-4">
                        <div class="bg-light rounded d-flex align-items-center justify-content-between p-4">
                            <i class="fa fa-users fa-3x text-dark"></i>
                            <div class="ms-3">
                                <p class="mb-2">Active Instructor</p>
                                <h6 class="mb-0"><?php echo $data['total'] ?></h6>
                            </div>
                        </div>
                    </div>
<?php 
    $select = "select COUNT(*) as total from instructor_registration where user_status = 'Block'";
    $res = mysqli_query($con, $select);
    $data = mysqli_fetch_array($res);
    if($data['total'] == ''){
        $data['total'] = 0;
...
?>
                    <div class="col-sm-6 col-xl-4">
                        <div class="bg-light rounded d-flex align-items-center justify-content-between p-4">
                            <i class="fa fa-users fa-3x text-dark"></i>
                            <div class="ms-3">
                                <p class="mb-2">Block Instructor</p>
                                <h6 class="mb-0"><?php echo $data['total'] ?></h6>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="widget-header">                                
                                    <div class="row">
                                        <div class="col-xl-12 col-md-12 col-sm-12 col-12 mt-5">
                                            <center><h4>Instructor Panel Link</h4> </center>
                                        </div>
                                    </div>
...
<div class="clipboard">
                        ...
                        <input type="text" class="form-control mb-4" id="referal_link" value="<?php echo $instructor_link; ?>" readonly="">
